            ...
            <section className="relative overflow-hidden bg-theme-black">
                ...
                <div className="container relative z-10 px-4 py-16 md:px-6">
                    <div className="mx-auto max-w-4xl text-center">
                        <h1 className="mt-4 text-4xl font-bold tracking-tight sm:text-5xl"><span className="bg-gradient-to-r from-theme-yellow to-theme-red bg-clip-text text-transparent">Student Body Government (SBG)</span></h1>
                        <p className="mt-4 text-xl text-muted-foreground">{sbgData.description}</p>
                    </div>
                </div>
            </section >
                            ...
                            <div className="relative lg:flex lg:justify-between lg:items-center p-6">
                                ...
                                <div className="group w-fit max-sm:w-full max-sm:mt-8 max-sm:p-2 md:p-4 relative overflow-hidden rounded-2xl border border-theme-gray-light bg-theme-gray transition-all hover:scale-105 hover:shadow-lg">
                                    ...
                                    <div className="relative flex justify-center items-center gap-4 max-sm:flex-col max-sm:items-center">
                                        <div className="inline-flex items-center rounded-full border border-theme-gray-light bg-theme-gray-light/30 text-sm backdrop-blur-sm"><Mail className="text-theme-yellow" /></div>
                                        <p className="text-wrap"><strong>{yearGroup.representatives.email.split('@').map((part, index) => (<span key={index} className="max-sm:block">{part}{index === 0 && <span className="max-sm:inline">@</span>}</span>))}</strong></p>
                                    </div>
                                </div>
                            </div>
                        </div>
